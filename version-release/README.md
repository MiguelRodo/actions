...
| `version` | Exact version to apply to all packages (e.g. 1.2.3). Overridden per-package by python_version or r_version. Cannot be set together with bump_type. | No | `""` |
| `bump_type` | Version component to bump (major \| minor \| patch). The new version is derived by bumping the most recent semver git tag. Cannot be set together with version. | No | `""` |
| `python_version` | Override: exact version to set for the Python package (e.g. 1.2.3). | No | `""` |
| `r_version` | Override: exact version to set for the R package (e.g. 1.2.3). | No | `""` |
