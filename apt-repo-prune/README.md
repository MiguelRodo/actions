...
<!-- action-inputs:start -->
<!-- Generated from apt-repo-prune/action.yml by scripts/generate-action-docs.py. Do not edit this block manually. -->
## Inputs

| Input | Description | Required | Default |
| --- | --- | :---: | --- |
| `token` | GitHub token with contents: write permission on the current repository. | Yes | — |
| `retention` | Version retention policy (case-insensitive). 'latest' – keep only the newest version per (package, arch) pair. 'latest-per-minor' – keep the highest patch release for each major.minor series. 'latest-per-major' (default) – keep the highest minor.patch release for each major series. | No | `latest-per-major` |
| `apt_signing_key` | Optional ASCII-armored GPG private key for signing the regenerated apt metadata. When omitted only the unsigned Release file is written and any stale InRelease / Release.gpg files are removed. | No | `""` |
| `apt_signing_key_passphrase` | Optional passphrase for apt_signing_key. | No | `""` |
<!-- action-inputs:end -->
